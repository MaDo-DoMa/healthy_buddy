...
class FocusApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.timer_label = ctk.CTkLabel(left, text=self._fmt(self._seconds_left), font=("Courier New", 72, "bold"),
        ...
        self.start_btn = ctk.CTkButton(btn_frame, text="▶ START", fg_color=COLORS["accent"], command=self._toggle_timer,
                                       corner_radius=10, width=130, height=42)
        self.start_btn.grid(row=0, column=0, padx=8)

        ctk.CTkButton(btn_frame, text="↺ RESET", fg_color=COLORS["border"], command=self._reset_timer, width=110,
                      height=42).grid(row=0, column=1, padx=8)
        ctk.CTkButton(btn_frame, text="⏭ SKIP", fg_color=COLORS["border"], command=self._skip_phase, width=110,
                      height=42).grid(row=0, column=2, padx=8)

        # Settings inputs
        settings_frame = ctk.CTkFrame(left, fg_color=COLORS["bg"], corner_radius=12, border_width=1,
                                      border_color=COLORS["border"])
        settings_frame.grid(row=4, column=0, padx=24, pady=(0, 24), sticky="sew")
        ...
        for col, (lbl, key) in enumerate(
            ...
            ctk.CTkLabel(settings_frame, text=lbl, font=("Courier New", 10), text_color=COLORS["muted"]).grid(row=0,
                                                                                                              column=col,
                                                                                                              pady=(10,
                                                                                                                    2))
        ...
        self.pom_label.grid(row=5, column=0, pady=(0, 16))

        # --- RIGHT PANEL: TO-DO ---
        right = ctk.CTkFrame(self, fg_color=COLORS["panel"], corner_radius=18, border_width=1,
                             border_color=COLORS["border"])
        right.grid(row=0, column=1, padx=(9, 18), pady=18, sticky="nsew")
        right.grid_columnconfigure(0, weight=1)
        right.grid_rowconfigure(2, weight=1)

        ctk.CTkLabel(right, text="TO-DO", font=("Courier New", 13, "bold"), text_color=COLORS["accent2"]).grid(row=0,
                                                                                                               column=0,
                                                                                                               pady=(22,
                                                                                                                     8))
        ...
        ctk.CTkButton(input_frame, text="+", width=36, command=self._add_task, fg_color=COLORS["accent2"]).grid(row=0,
                                                                                                                column=1)
        ...
        ctk.CTkButton(right, text="🗑 Usuń ukończone", font=("Courier New", 11), fg_color="transparent", border_width=1,
                      border_color=COLORS["border"], command=self._delete_done).grid(row=3, column=0, padx=16,
                                                                                     pady=(0, 16), sticky="ew")
    ...
    def _render_task(self, task_id, text, done, insert_top=False):
        row = ctk.CTkFrame(self.task_scroll, fg_color=COLORS["bg"], corner_radius=8, border_width=1,
                           border_color=COLORS["border"])
